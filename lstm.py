...
# Define a neural network control policy using LSTM
class LSTMPolicy(nn.Module):
    ...
    def forward(self, y, ymin, ymax, d_obs):
        # Concatenate inputs
        x = torch.cat([y, ymin, ymax, d_obs], dim=-1)
        # Ensure x is 3-D: (batch_size, sequence_length, input_size)
        if x.dim() == 2:
        ...
        return torch.clamp(out, self.min, self.max)  # Clamp output to control bounds

# Instantiate the LSTM policy network
net = LSTMPolicy(
    input_size=ny + 2*nref + nd_obs,
    hidden_size=32,
    output_size=nu,
    num_layers=2
)
...
action_loss = 0.01 * (u == torch.zeros_like(u))  # Energy minimization
du_loss = 0.1 * ((u[:, :-1, :] - u[:, 1:, :]) == torch.zeros_like(u[:, :-1, :]))  # Delta u minimization to prevent aggressive changes in control actions
...
# Debug: Check gradients after training
for name, param in net.named_parameters():
    if param.grad is not None:
        print(f"Layer {name} | Gradient: {param.grad.abs().mean()}")
    else:
